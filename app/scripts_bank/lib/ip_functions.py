...
def isSubnetMaskAHost(x):
    """Return True if subnet mask is for a single host, otherwise returns false."""
...
def validateSubnetMask(mask):
    """Validate input is a valid subnet mask.
...
def validatePortNumber(port):
    """Validate input is a valid port.
...
def validatePortProtocol(proto):
    """Validate input is a valid port protocol.
...
def incrementIPByOne(ip):
    """Increment an IP address by 1, then returns it to user (ex: 10.0.0.0 becomes 10.0.0.1)."""
    # Split IP by '.'
    aList = ip.split('.')
    # Get last octect
    a = aList[-1]
