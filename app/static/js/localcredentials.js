$('#modalLocalCredentials').on('show.bs.modal', function(event) {
  ...
  // Check to see if device has a currently established and active SSH session.
  // If so, do not prompt user for credentials.  Show loading screen modal while loading device on existing session.
  // If not, show modal prompting user for local credentials for the specific device.
  $.ajax({
    type: 'POST',
    url: '/ajaxcheckdeviceactivesshsession/' + deviceid,
