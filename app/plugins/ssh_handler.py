    ...
    def retrieve_ssh_session(self, host, saved_session=True):
        ...
        if saved_session:
            if not self.check_host_existing_ssh_session(host):
                app.logger.info('initiated new SSH connection to %s' % (host.hostname))
            ...
            # Run test to verify if socket connection is still open or not
                ...
                app.logger.info('reestablished SSH connection to %s' % (host.hostname))
                self.ssh[sshKey] = get_ssh_session(host, creds)
    ...
    def disconnect_specific_ssh_session(self, host):
        ...
        for x in self.ssh:
            ...
            if int(y[0]) == int(host.id):
                ...
                self.ssh.pop(x)
                app.logger.info('disconnected SSH session to provided host %s from user %s' % (host.hostname, session['USER']))
    ...
    def disconnect_all_ssh_sessions(self):
        ...
        for x in self.ssh:
            ...
            if str(y[1]) == str(session['UUID']):
                ...
                app.logger.info('disconnected SSH session to device %s for user %s' % (host.hostname, session['USER']))
